define(["./cyclicJSON"], function(cycle) {
  ...
  Grammar.fromSerializable = function(obj) {
    ...
    for (var i = 0; i < obj.eSPPFs.length; i++) {
      ...
      if (src.kids) {
        ...
        dest.addChildren(rulesByOldId[src.rule], kids, g);
      }
    }
    return g;
  }
  ...
  Grammar.prototype = {
    ...
    applyAction: function(rule, kids, pos, semActions) {
      ...
      var ans = action.call(rule, kids, pos, semActions);
      var flags = this.getFlags(rule);
      if (this.ambiguitySortingFlag) {
        var total = flags ? flags[this.ambiguitySortingFlag] || 0 : 0;
        for (var i = 0; i < kids.length; i++) {
          total += kids[i][this.ambiguitySortingFlag] || 0;
        }
        ans[this.ambiguitySortingFlag] = total;
      }
      if (flags) ans.flags = flags;
      return ans;
    },
    ...
    */
    ...
    countAndPriceAllParses: function(sppfNode) {
      if (sppfNode === undefined) return;
      var todo = [{todo: [sppfNode], done: [], type: "sum"}]; // stack of alternate lists of children
      var costKey = this.ambiguitySortingFlag;
      while (todo.length > 0) {
        var top = todo[todo.length - 1];
        if (top.todo.length > 0) {
          var first = top.todo[top.todo.length - 1];
          if (first.minCost !== undefined) { // already processed, because parse is a DAG
            top.todo.pop();
            top.done.push(first);
          } else if (first.ambig) {
            todo.push({todo: Array.prototype.slice.call(first.ambig), done: [], type: "sum"});
          } else if (first.kids && first.kids.length > 0) {
            todo.push({todo: Array.prototype.slice.call(first.kids), done: [], type: "prod"});
          } else if (first.kids) {
            top.todo.pop();
            top.done.push(first);
            first.count = 1;
            var flags = this.getFlags(first.rule);
            first.minCost = flags ? flags[costKey] || 0 : 0;
            first.maxErrPos = first.minCost > 0 ? first.pos.startChar : Number.MAX_VALUE;
          } else {
            top.todo.pop();
            top.done.push(first);
            first.minCost = 0;
            first.count = 1;
            first.maxErrPos = Number.MAX_VALUE;
          }
        } else {
          todo.pop();
          if (todo.length === 0) break;
          var next = todo[todo.length - 1];
          var first = next.todo[next.todo.length - 1];
          next.todo.pop();
          next.done.push(first);
          if (top.type === "sum") {
            var count = 0;
            var cost = Number.MAX_VALUE;
            var maxErrPos = Number.MAX_VALUE;
            for (var i = 0; i < top.done.length; i++) {
              count += top.done[i].count;
              cost = Math.min(cost, top.done[i].minCost);
              maxErrPos = Math.min(maxErrPos, top.done[i].maxErrPos);
            }
            first.minCost = cost;
            first.count = count;
            first.maxErrPos = maxErrPos;
          } else {
            var flags = first.rule ? this.getFlags(first.rule) : undefined;
            var cost = flags ? flags[costKey] || 0 : 0;
            var count = top.done.length > 0 ? 1 : 0;
            var maxErrPos = cost > 0 ? first.pos.startChar : Number.MAX_VALUE;
            for (var i = 0; i < top.done.length; i++) {
              count *= top.done[i].count;
              cost += top.done[i].minCost;
              if (top.done[i].minCost > 0)
                maxErrPos = Math.min(maxErrPos, top.done[i].pos.endChar);
            }
            first.minCost = Math.min(first.minCost || Number.MAX_VALUE, cost);
            first.count = count;
            first.maxErrPos = Math.min(first.maxErrPos || Number.MAX_VALUE, maxErrPos);
          }
        }
      }
    },
      ...
      while (stack.length > 0 && stack[0].todo.length > 0) {
        ...
        if (curr.todo.length > 0) { 
          ...
          if (next.label instanceof Token) {
            ...
            curr.todo.pop();
            var pos = curr.done.length > 0 ? curr.done[curr.done.length - 1].pos.posAtEnd() : next.pos.posAtStart();
    ...
    constructCheapestParse: function(sppfNode, semActions) {
      if (sppfNode === undefined) return undefined;
      // Each stack item is a pair {todo, done}, where todo is a list of children
      // in reverse order (so that pop goes from left-to-right in actual tree order)
      // and done is a list of children in tree order
      ...
      function endPos(node) {
        return node.kids[node.kids.length - 1].pos.endChar;
      }
      function findBestAlternative(alts) {
        var minCost = Number.MAX_VALUE;
        var best = undefined;
        for (var i = 0; i < alts.length; i++) {
          if (alts[i].minCost < minCost) {
            minCost = alts[i].minCost;
            if (best === undefined) best = alts[i];
          }
        }
        for (var i = 0; i < alts.length; i++) {
          if (alts[i].minCost === minCost) {
            if (minCost === 0) { return alts[i]; }
            else if (alts[i].maxErrPos > best.maxErrPos) {
              best = alts[i];
            }
          }
        }
      ...
      while (stack.length > 0 && stack[0].todo.length > 0) {
        ...
        if (curr.todo.length > 0) { 
          ...
          if (next.label instanceof Token) {
            ...
            curr.todo[curr.todo.length - 1] = findBestAlternative(next.ambig);
          } else if (next.rule === undefined && next.label.indexOf(',') > -1) {
            curr.todo.pop();
            for (var k = next.kids.length - 1; k >= 0; k--)
              curr.todo.unshift(next.kids[k]);
    ...
    constructNextParse: function(sppfNode, semActions, directions) {
      ...
      if (directions) {
        ...
        while (lastIndex >= 0 && directions[lastIndex].choice == directions[lastIndex].of - 1)
          lastIndex--;
        ...
        if (curr.todo.length > 0) {
          ...
          if (next.label instanceof Token) {
            ...
            if (curDir < newDirs.length) {
              ...
              if (next === undefined) {
                throw "Couldn't find next!";
              }
              curr.todo[curr.todo.length - 1] = next;
            } else {
              newDirs.push({choice: -1, of: next.ambig.length});
              return this.constructNextParse(sppfNode, semActions, newDirs);
            }
          } else if (next.kids.length == 0) {
            // Optimization for nullary nonterminals
            curr.todo.pop();
          ...
          if (stack.length > 1) {
            ...
            if (toReduce.rule === undefined) {
              if (toReduce.label.indexOf(',') > -1) {
                ...
                function jsonPos(key, value) {
                  if (key === "pos") { return "SrcLoc(" + value.toString(true) + ")"; }
                  else if (key === "label" && typeof value === "object") { return value.key; }
                  else if (key === "rule" && value) { return "Rule(" + value.asString + ")"; }
                  else if (key === "rule") { return "NO RULE!"; }
                  else { return value; }
                }
                throw("Cannot find a reduction rule on " + JSON.stringify(toReduce, jsonPos, "  "));
              }
