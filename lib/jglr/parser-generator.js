...
R(['lib/jglr/jglr', 'fs'], function(E, fs) {
  ...
  toks.makeToken = function(tok_type, s, pos) {
    switch(tok_type) {
    case "STRING": s = fixEscapes(s.slice(1, -1)); break;
    }
    return Tokenizer.prototype.makeToken(tok_type, s, pos);
  }
  ...
  g.addRule("Flag", [new Token("NAME"), new Token("COLON"), new Token("STRING")]);
  ...
  function generateRules(rules, config) {
    ...
    if (config.sortKey) {
      ret.push("g.sortAmbiguitiesBy(" + JSON.stringify(config.sortKey) + ");");
    }
    for (var i = 0; i < rules.kids.length; i++) {
  ...
  function generateFlags(flags) {
    ...
    for (var i = 0; i < flags.kids.length; i++) {
      ...
      if (isNaN(valAsNum) || flags.kids[i].kids[2].name === "STRING")
        ret[key] = val;
      else
        ret[key] = valAsNum;
    }
